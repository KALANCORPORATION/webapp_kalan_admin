    ...
    const handleReservation = async () => {
        try {
            const borrowPromises = books.map(book => {
                const formattedStartDate = startDates[book.book.id] ? reformatDate(startDates[book.book.id]) : "default_start_date";
                const formattedEndDate = endDates[book.book.id] ? reformatDate(endDates[book.book.id]) : "default_end_date";
                const reservationData = { start_date: formattedStartDate, end_date: formattedEndDate, adherent_id: id };
                return ReservationController.createReservation(book.id, reservationData, accessToken);
            });
                    ...
                    <div className="book-list">
                        {books.map((book, index) => (
                            <div key={book.book.id} className="book-list-item">
                                ...
                                <input
                                    type="date"
                                    value={startDates[book.book.id] || ''}
                                    onChange={(e) => setStartDates({ ...startDates, [book.book.id]: e.target.value })}
